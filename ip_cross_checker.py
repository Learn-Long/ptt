...
def main():
    ...
    print(f"Found {len(json_files)} valid JSON files within 7 days.")
                        ...
                        for ip, user_urls in shared_ips_and_urls.items():
                            ...
                            for user in users:
                                ...
                                for url in unique_urls:
                                    outfile.write(f"    - {url}\n")
                                # --- End of Modified URL writing logic ---
                            outfile.write("-" * 30 + "\n")
            else:
    ...
    except Exception as e: # Added to fix Pylance error
            ...
            if shared_ip_details:
                ...
                for item in shared_ip_details:
                    ...
                    for user_detail in details:
                        ...
                        unique_urls = []
                        previous_url = None
                        for url in sorted(user_detail['urls']):
                            if url != previous_url:
                                unique_urls.append(url)
                            previous_url = url
                        for url in unique_urls:
